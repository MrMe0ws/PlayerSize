...
- ✅ Команды управления: `/playersize set`, `/playersize reset`, `/playersize check`, `/playersize list`, `/playersize give`
...
### `/playersize give potion [игрок] <количество>`

Выдает зелье изменения роста указанному игроку (или себе, если игрок не указан). Если инвентарь полон, зелья выкидываются на землю.

**Права доступа:** `playersize.admin` (только админы/OP)

**Параметры:**

- `[игрок]` - ник игрока (необязательно, если команда выполняется игроком)
- `<количество>` - количество зелий (от 1 до 64)

**Использование:**

```
/playersize give potion 5
/playersize give potion PlayerName 10
```

**Примеры:**

- `/playersize give potion 5` - выдать себе 5 зелий роста
- `/psize give potion PlayerName 10` - выдать игроку PlayerName 10 зелий
- `/psz give potion 64` - выдать себе максимум зелий (64 штуки)

**Особенности:**

- Если инвентарь полон, зелья выкидываются на землю
- Команда работает только если зелье включено в конфиге (`potion.enabled: true`)
- Консоль должна указать игрока: `/playersize give potion <игрок> <количество>`
